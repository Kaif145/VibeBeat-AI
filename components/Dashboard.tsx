...
const Dashboard: React.FC<DashboardProps> = ({ user, onMoodSubmit, onPhotoSubmit, analysis, analysisSource, onClearAnalysis, onSaveTrack }) => {
  ...
  const isSaved = (title: string, artist: string) => {
    // Generate a consistent ID for virtual tracks
  ...
  if (analysis) {
    // Logic to handle correct labeling based on user action
        ...
        <div className="max-w-3xl mx-auto space-y-8 pb-20">
          <div className="flex items-center justify-between">
            <button 
              ...
              className="text-neutral-900 font-black uppercase tracking-widest text-[10px] flex items-center gap-2 hover:opacity-70 transition-all active:scale-95"
            >
          ...
          <div className="bg-white border border-neutral-200 rounded-[40px] p-6 md:p-8 shadow-sm flex flex-col items-center gap-6 overflow-hidden">
            ...
            <button 
              ...
              className="bg-black text-white font-black py-4 px-10 rounded-full flex items-center gap-2 hover:scale-105 transition-all shadow-xl shadow-black/10 uppercase tracking-widest text-xs active:scale-95"
            >
          ...
          <div className="space-y-6">
            ...
            {analysis.recommendedTracks.map((track, i) => (
              <div key={i} className="bg-white border border-neutral-200 rounded-[40px] p-6 md:p-10 shadow-sm space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-700 hover:border-purple-300 transition-all group" style={{ animationDelay: `${i * 70}ms` }}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill={isSaved(track.title, track.artist) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
                  </button>
                </div>
                ...
                <div className="pt-2">
                  <a 
                    ...
                    className="w-full bg-[#1DB954] text-white font-black py-5 px-8 rounded-full flex items-center justify-center gap-4 hover:bg-[#1ed760] hover:scale-[1.02] transition-all shadow-xl shadow-green-500/10 uppercase tracking-widest text-xs active:scale-95"
                  >
            ...
            <button 
              ...
              className="w-full py-12 border-2 border-dashed border-neutral-200 rounded-[40px] text-neutral-300 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-white hover:border-purple-300 hover:text-purple-500 transition-all duration-700 bg-neutral-50/30 active:scale-95"
            >
